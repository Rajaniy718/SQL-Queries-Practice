...
SELECT *
FROM [dbo].[comprehensive_mutual_funds_data]
WHERE returns_3yr IS NULL OR returns_3yr = '' -- Handle missing numeric values separately
...
--missing values-- category ,sub_category ,amc_name , scheme_name, fund_manager, returns_3yr, returns_5yr, min_sip , 
...
SELECT *
From [dbo].[comprehensive_mutual_funds_data]
WHERE category = 'Hybrid' AND amc_name = 'JM Financial Mutual Fund'


SELECT scheme_name, min_sip , min_lumpsum, expense_ratio, fund_size_cr, fund_age_yr, alpha, sd, beta, sharpe, risk_level, rating,returns_1yr, returns_3yr, returns_5yr
FROM [dbo].[comprehensive_mutual_funds_data]
...
SELECT DISTINCT(amc_name) , count(amc_name) as 'total_count'  --------AMC market share: top three amc's
FROM [dbo].[comprehensive_mutual_funds_data]
GROUP BY amc_name
ORDER BY 2 DESC

SELECT TOP 3 amc_name , count(amc_name) as'total_count' ---- top 3 with values
FROM [dbo].[comprehensive_mutual_funds_data]
GROUP BY amc_name
ORDER BY COUNT(*) DESC;
